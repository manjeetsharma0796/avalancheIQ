...
const Navbar = () => {
  ...
  return (
    <nav className="absolute top-0 left-0 right-0 flex justify-between items-center p-6 bg-transparent z-50">
      <div 
        ref={logoRef} 
        className="flex items-center font-bold text-3xl text-transparent bg-clip-text bg-gradient-to-r from-red-500 to-pink-500" // Updated gradient
      >
        <a href="/">AvalancheIQ</a>
      </div>
      <button 
        onClick={isConnected ? undefined : connectWallet} 
        className="text-white bg-gradient-to-r from-red-500 to-red-700 hover:from-red-400 hover:to-red-600 transition duration-300 px-4 py-2 rounded-lg transform hover:scale-105" // Updated button gradient
      >
